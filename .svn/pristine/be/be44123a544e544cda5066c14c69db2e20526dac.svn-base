...
class PO_Ajax {
	...
	function save_order() {
		...
		if ( current_user_can( 'activate_plugins' ) ) {
			...
			if (preg_match("/^(([0-9])+[,]*)*$/", implode(",", $this->postedData['orderList'])) && preg_match("/^(([0-9])+[,]*)*$/", implode(",", $this->postedData['startOrder']))) {
				$newPlugArray = $this->postedData['orderList'];
				$startOrderArray = $this->postedData['startOrder'];
	...
	function create_group() {
		...
		if (is_array($this->postedData['PO_group_list']) && $this->PO->validate_field('PO_group_name')) {
			$groupID = wp_insert_post(array('post_title'=>$this->postedData['PO_group_name'], 'post_type'=>'plugin_group', 'post_status'=>'publish'));
			if (!is_wp_error($groupID)) {
				update_post_meta($groupID, '_PO_group_members', $this->postedData['PO_group_list']);
	...
	function edit_plugin_group_name() {
		...
		if (is_numeric($this->postedData['PO_group_id']) && $this->PO->validate_field('PO_group_name')) {
			$oldGroupTitle = get_the_title($this->postedData['PO_group_id']);
			$post_id = wp_update_post(array('ID'=>$this->postedData['PO_group_id'], 'post_title'=>$this->postedData['PO_group_name']));
			if ($post_id > 0) {
				$newGroupTitle = get_the_title($this->postedData['PO_group_id']);
	...
	function save_global_plugins() {
		...
		if ( current_user_can( 'activate_plugins' ) ) {
			if (isset($this->postedData['PO_disabled_std_plugin_list']) && is_array($this->postedData['PO_disabled_std_plugin_list']) && $this->postedData['PO_disabled_std_plugin_list'][0] != 'EMPTY') {
				$disabledPlugins = $this->postedData['PO_disabled_std_plugin_list'];
			} else {
			...
			if (isset($this->postedData['PO_disabled_std_group_list']) && is_array($this->postedData['PO_disabled_std_group_list']) && $this->postedData['PO_disabled_std_group_list'][0] != 'EMPTY') {
				$disabledGroups = $this->postedData['PO_disabled_std_group_list'];
			} else {
			...
			if (get_option('PO_disable_plugins_mobile') == 1) {
				if (isset($this->postedData['PO_disabled_mobile_plugin_list']) && is_array($this->postedData['PO_disabled_mobile_plugin_list']) && $this->postedData['PO_disabled_mobile_plugin_list'][0] != 'EMPTY') {
					$disabledMobilePlugins = $this->postedData['PO_disabled_mobile_plugin_list'];
				} else {
				...
				if (isset($this->postedData['PO_disabled_mobile_group_list']) && is_array($this->postedData['PO_disabled_mobile_group_list']) && $this->postedData['PO_disabled_mobile_group_list'][0] != 'EMPTY') {
					$disabledMobileGroups = $this->postedData['PO_disabled_mobile_group_list'];
				} else {
	...
	function save_pt_plugins() {
		...
		if (isset($this->postedData['selectedPostType']) && $this->postedData['selectedPostType'] != '') {
			$postType = $this->postedData['selectedPostType'];
		} else {
		...
		if (is_numeric($this->postedData['PO_post_offset'])) {
			$returnVals['offset'] = $this->postedData['PO_post_offset'];
		} else {
		...
		if (!is_numeric($this->postedData['PO_total_post_count']) || $this->postedData['PO_total_post_count'] == 0) {
			...
			$returnVals['total'] = $this->postedData['PO_total_post_count'];
		}
		...
		$returnVals['msg'] = "Plugins updated for " . $this->postedData['selectedPostType'] . ".";
		print json_encode($returnVals);
		die();
	}
	...
	function redo_permalinks() {
		...
		if (!empty($this->postedData['old_site_address'])) {
			$oldSiteAddress = preg_quote($this->PO->fix_trailng_slash($this->postedData['old_site_address']), "/");
		} else {
			$oldSiteAddress = "";
		}

		if (!empty($this->postedData['new_site_address'])) {
			$newSiteAddress = $this->PO->fix_trailng_slash($this->postedData['new_site_address']);
		} else {
			$newSiteAddress = "";
		}
		...
		$postIDsQuery = preg_replace('/\[ESC_LIKE\]/', "'".$wpdb->esc_like($this->postedData['new_site_address'])."%'", $postIDsQuery);
	...
	function save_mobile_user_agents() {
		...
		$userAgents = preg_replace("/\\r\\n/", "\n", $this->postedData['PO_mobile_user_agents']);
		$userAgents = explode("\n", $userAgents);
	...
	function submit_admin_css_settings() {
		...
		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_network_plugins_bg_color'])) {
			$POAdminStyles['network_plugins_bg_color'] = $this->postedData['PO_network_plugins_bg_color'];
		} else {
			$result .= "The text submitted for Network Plugin Background is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_network_plugins_font_color'])) {
			$POAdminStyles['network_plugins_font_color'] = $this->postedData['PO_network_plugins_font_color'];
		} else {
			$result .= "The text submitted for Network Plugin Font is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_active_plugins_bg_color'])) {
			$POAdminStyles['active_plugins_bg_color'] = $this->postedData['PO_active_plugins_bg_color'];
		} else {
			$result .= "The text submitted for Active Plugin Background is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_active_plugins_font_color'])) {
			$POAdminStyles['active_plugins_font_color'] = $this->postedData['PO_active_plugins_font_color'];
		} else {
			$result .= "The text submitted for Active Plugin Font is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_active_plugins_border_color'])) {
			$POAdminStyles['active_plugins_border_color'] = $this->postedData['PO_active_plugins_border_color'];
		} else {
			$result .= "The text submitted for Active Plugin Border is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_inactive_plugins_bg_color'])) {
			$POAdminStyles['inactive_plugins_bg_color'] = $this->postedData['PO_inactive_plugins_bg_color'];
		} else {
			$result .= "The text submitted for Inactive Plugin Background is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_inactive_plugins_font_color'])) {
			$POAdminStyles['inactive_plugins_font_color'] = $this->postedData['PO_inactive_plugins_font_color'];
		} else {
			$result .= "The text submitted for Inactive Plugin Font is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_inactive_plugins_border_color'])) {
			$POAdminStyles['inactive_plugins_border_color'] = $this->postedData['PO_inactive_plugins_border_color'];
		} else {
			$result .= "The text submitted for Inactive Plugin Border is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_plugin_groups_bg_color'])) {
			$POAdminStyles['plugin_groups_bg_color'] = $this->postedData['PO_plugin_groups_bg_color'];
		} else {
			$result .= "The text submitted for Plugin Group Background is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_plugin_groups_font_color'])) {
			$POAdminStyles['plugin_groups_font_color'] = $this->postedData['PO_plugin_groups_font_color'];
		} else {
			$result .= "The text submitted for Plugin Group Font is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_plugin_groups_border_color'])) {
			$POAdminStyles['plugin_groups_border_color'] = $this->postedData['PO_plugin_groups_border_color'];
		} else {
			$result .= "The text submitted for Plugin Group Border is not a valid css hex color.<br />";
		}
		
		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_global_plugins_bg_color'])) {
			$POAdminStyles['global_plugins_bg_color'] = $this->postedData['PO_global_plugins_bg_color'];
		} else {
			$result .= "The text submitted for Global Plugin Background is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_global_plugins_font_color'])) {
			$POAdminStyles['global_plugins_font_color'] = $this->postedData['PO_global_plugins_font_color'];
		} else {
			$result .= "The text submitted for Global Plugin Font is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_global_plugins_border_color'])) {
			$POAdminStyles['global_plugins_border_color'] = $this->postedData['PO_global_plugins_border_color'];
		} else {
		...
		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_selected_plugins_bg_color'])) {
			$POAdminStyles['selected_plugins_bg_color'] = $this->postedData['PO_selected_plugins_bg_color'];
		} else {
			$result .= "The text submitted for Selected Plugin Background is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_selected_plugins_font_color'])) {
			$POAdminStyles['selected_plugins_font_color'] = $this->postedData['PO_selected_plugins_font_color'];
		} else {
			$result .= "The text submitted for Selected Plugin Font is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_selected_plugins_border_color'])) {
			$POAdminStyles['selected_plugins_border_color'] = $this->postedData['PO_selected_plugins_border_color'];
		} else {
		...
		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_on_btn_bg_color'])) {
			$POAdminStyles['on_btn_bg_color'] = $this->postedData['PO_on_btn_bg_color'];
		} else {
			$result .= "The text submitted for On Button Background is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_on_btn_font_color'])) {
			$POAdminStyles['on_btn_font_color'] = $this->postedData['PO_on_btn_font_color'];
		} else {
			$result .= "The text submitted for On Button Font is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_off_btn_bg_color'])) {
			$POAdminStyles['off_btn_bg_color'] = $this->postedData['PO_off_btn_bg_color'];
		} else {
			$result .= "The text submitted for Off Button Background is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_off_btn_font_color'])) {
			$POAdminStyles['off_btn_font_color'] = $this->postedData['PO_off_btn_font_color'];
		} else {
			$result .= "The text submitted for Off Button Font is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_yes_btn_bg_color'])) {
			$POAdminStyles['yes_btn_bg_color'] = $this->postedData['PO_yes_btn_bg_color'];
		} else {
			$result .= "The text submitted for Yes Button Background is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_yes_btn_font_color'])) {
			$POAdminStyles['yes_btn_font_color'] = $this->postedData['PO_yes_btn_font_color'];
		} else {
			$result .= "The text submitted for Yes Button Font is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_no_btn_bg_color'])) {
			$POAdminStyles['no_btn_bg_color'] = $this->postedData['PO_no_btn_bg_color'];
		} else {
			$result .= "The text submitted for No Button Background is not a valid css hex color.<br />";
		}

		if (preg_match("/^(#[0-9A-Za-z]{0,6}|)$/", $this->postedData['PO_no_btn_font_color'])) {
			$POAdminStyles['no_btn_font_color'] = $this->postedData['PO_no_btn_font_color'];
		} else {
	...
	function reset_post_settings() {
		...
		if (is_numeric($this->postedData['postID'])) {
			if ($wpdb->get_var($wpdb->prepare("SELECT count(*) FROM ".$wpdb->prefix."po_plugins WHERE post_id=%d", $this->postedData['postID'])) > 0) {
				...
				if ($wpdb->query($wpdb->prepare($deletePluginQuery, $this->postedData['postID'])) === false) {
					print 0;
				} else {
					print 1;
				}
	...
	function save_gen_settings() {
		...
		##Fuzzy URL matching
		if ($this->postedData['PO_fuzzy_url_matching'] == "1") {
		...
		##Ignore protocol
		if ($this->postedData['PO_ignore_protocol'] == "1") {
		...
		##Ignore arguments
		if ($this->postedData['PO_ignore_arguments'] == "1") {
		...
		if ($this->postedData['PO_order_access_net_admin'] == "1") {
		...
		##Auto trailing slash
		if ($this->postedData['PO_auto_trailing_slash'] == "1") {
		...
		if (isset($this->postedData['PO_cutom_post_type']) && is_array($this->postedData['PO_cutom_post_type'])) {
			$submittedPostTypes = $this->postedData['PO_cutom_post_type'];
		} else {
		...
		if (isset($this->postedData['PO_supported_roles']) && is_array($this->postedData['PO_supported_roles'])) {
			$submittedRoles = $this->postedData['PO_supported_roles'];
		} else {
		...
		if ($this->postedData['PO_disable_plugins_frontend'] == "1") {
		...
		if ($this->postedData['PO_disable_plugins_mobile'] == "1") {
		...
		if ($this->postedData['PO_disable_plugins_admin'] == "1") {
		...
		##Disable By Role
		if ($this->postedData['PO_disable_plugins_by_role'] == "1") {
	...
	function get_plugin_group_container() {
		...
		foreach($groups as $group) {
			...
			$members = stripslashes_deep($members);
			if (is_array($members) && array_search($this->postedData['PO_plugin_path'], $members) !== FALSE) {
	...
	function perform_plugin_search() {
		...
		if (isset($this->postedData['PO_searchable_roles']) && is_array($this->postedData['PO_searchable_roles']) && count($this->postedData['PO_searchable_roles']) > 0) {
			$availableRoles = $this->postedData['PO_searchable_roles'];
		...
		$sql = preg_replace('/\[ESC_LIKE\]/', "'%".$wpdb->esc_like($this->postedData['PO_plugin_path'])."%'", $sql);
