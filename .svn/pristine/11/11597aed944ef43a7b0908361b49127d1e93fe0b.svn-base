...
define('MAPSVG_VERSION', '1.5.5');
define('MAPSVG_JQUERY_VERSION', '5.5.15');
...
function mapsvg_add_jscss_common(){
    ...
    //wp_register_script('mapsvg', MAPSVG_PLUGIN_URL . 'js/mapsvg.js', array('jquery'), MAPSVG_JQUERY_VERSION);
...
function mapsvg_add_jscss_admin($hook_suffix){
    ...
    if(isset($_GET['page']) && $_GET['page']=='mapsvg-config'){
        ...
        wp_register_script('mapsvg.admin', MAPSVG_PLUGIN_URL . 'js/admin.js', array('jquery'), '2.1');
        ...
        wp_register_script('bootstrap.min.js', MAPSVG_PLUGIN_URL . 'js/bootstrap.min.js');
        ...
        // Remove all jQuery-UIs because they conflict with Bootstrap
        wp_deregister_script('jquery-ui');
        wp_deregister_script('jquery-ui-resizable');
        wp_deregister_script('jquery-ui-button');
        wp_deregister_script('jquery-ui-position');
        wp_deregister_script('jquery-ui-dialog');
        wp_deregister_script('wp-jquery-ui-dialog');
...
function mapsvg_print( $atts ){
  ...
  $data  = '<div id="mapsvg-'.$post->ID.'" class="mapsvg"></div><script type="text/javascript">';
  ...
  if(!empty($atts['selected'])){
      ...
      $data .= 'jQuery("#mapsvg-'.$post->ID.'").mapSvg('.$post->post_content.');</script>';
  }
...
function mapsvg_save( $data ){
    ...
    if(isset($data['map_id']) && $data['map_id']!='new'){
        ...
        mysql_query("update $wpdb->posts set post_title='".$postarr['post_title']."',
                     post_content = '".$postarr['post_content']."'
                     WHERE ID = ".$post_id, $wpdb->dbh);
...
function mapsvg_copy($id, $new_name){
    ...
    mysql_query("update $wpdb->posts set post_title='".$new_name."',
                 post_content = '".$copy_post['post_content']."'
                 WHERE ID = ".$new_id, $wpdb->dbh);
...
function mapsvg_conf(){
    ...
    if($file){
        ...
        */
        ...
        $events = array('onClick'=>null,'mouseOver'=>null, 'mouseOut'=>null);
